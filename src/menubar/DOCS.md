...
- **Resize cursors on panel edges — canonical `resetCursorRects` pattern**: `_PanelContentView(NSView)` is installed as `panel.contentView()`. Its `resetCursorRects()` registers four cursor rects via `addCursorRect_cursor_`: bottom 8pt strip → `resizeUpDownCursor`, left/right 8pt strips → `resizeLeftRightCursor`, interior remainder → `arrowCursor`. AppKit calls `resetCursorRects` on window activation and after every resize — no manual invalidation needed. **Why `mouseMoved_`/`NSTrackingArea` failed (v1–v2):** `NSCursor.set()` called in `mouseMoved_` is immediately overridden by child-view cursor rects — AppKit processes child `resetCursorRects` *after* mouse-moved event dispatch. Specifically: `NSTextField` (project-name separator labels) installs I-Beam over its full frame by default; `NSButton` (footer) installs Arrow. Left/right edge entry lands over an NSTextField → I-Beam wins. Bottom edge entry lands over footer NSButton → Arrow appeared to work coincidentally. Fix has two parts: (1) `_PanelContentView.resetCursorRects` defines the authoritative edge zones at the contentView level; (2) `_CursorlessLabel(NSTextField)` subclass overrides `resetCursorRects` with a no-op — used for all `labelWithString_` labels in `_make_header_label` and `_make_separator_view` — prevents I-Beam installation from these display-only labels. `panel.disableCursorRects()` and `panel.setAcceptsMouseMovedEvents_(True)` removed (no longer needed).
