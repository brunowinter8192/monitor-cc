...
**Purpose:** `CCMenuBarApp` rumps subclass + `_PanelController` (NSObject target for panel toggle/focus/restart) + NSPanel sticky-toggle dropdown with dynamic height + timer + blink + `_rebuild_panel` + `_update_panel_inplace` + `_truncate_and_height` + `_resize_panel` + `_focus_session` + `_register_hotkey` + settings load/save + Auto-Jump toggle + `run()` entry point.
