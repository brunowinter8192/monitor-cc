...
- **Worker name from `basename(cwd)`, not encoded dir** (`discover.py:_process_project_dir`): `encode_project_path` (`session_finder.py:70`) replaces `_` with `-` — lossily. The `worker_name` extracted from the `~/.claude/projects/` encoded-dir via `_classify_encoded_dir` therefore has underscores replaced with hyphens. Using it for tmux-session reconstruction produces `worker-{proj}-{name-with-hyphen}` which doesn't match the real tmux session `worker-{proj}-{name_with_underscore}` → `_tmux_session_exists` returns False → worker dropped. Fix: `display_name = os.path.basename(cwd)` when cwd is a valid worktree path; cwd comes from JSONL and preserves the real filesystem name. Affects both `_worker_tmux_session` call and `SessionInfo.name`. Fallback (cwd unavailable): lossy name from encoded dir — acceptable since no tmux check occurs in that branch.
