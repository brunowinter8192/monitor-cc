...
_BG_LAUNCH_ACK_MARKER_3 = 'moved to the background (ID'
...
_BG_LAUNCH_ACK_PREFIX_3 = 'Command did not complete within its'
...
_BG_AUTO_TIMEOUT_MSG_MAIN = (
    'Command exceeded its timeout and was moved to the background. Do NOT check, poll, or read '
    'its output, and do NOT arm another background timer — go idle now and wait; you will get a '
    'completion notice for this exact task ID when it finishes.'
)

_ACK_ID_RE = re.compile(r'with ID:\s*([^.]*)\.')
_ACK_ID_PAREN_RE = re.compile(r'\(ID:\s*([^.)]*)\)')
...
def _build_launch_ack_replacement(ack_text, is_main=False):
    id_match = _ACK_ID_RE.search(ack_text) or _ACK_ID_PAREN_RE.search(ack_text)
    path_match = _ACK_PATH_RE.search(ack_text)
    task_id = id_match.group(1).strip() if id_match else ''
    path = path_match.group(1).strip() if path_match else ''
    if _is_bg_auto_timeout_ack(ack_text):
        lines = [_BG_AUTO_TIMEOUT_MSG_MAIN if is_main else _BG_AUTO_TIMEOUT_MSG]
    else:
