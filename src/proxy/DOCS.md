...
### strip_bg_completed.py (86 LOC)

**Purpose:** Replace the first background-Bash kill notification in user-turn content with a plain-text wake-up hint (`"worker idle"`); strip any further ones. CC injects `Background command “…” failed/completed (exit code 143/137)` when a background Bash process is terminated via SIGTERM (143) or SIGKILL (137). Instead of silently removing this, the proxy repurposes it: the first match is replaced with `_WAKEUP_TEXT` (`'worker idle\n'`), subsequent duplicates in the same traversal are stripped. Traverses all 4 content shapes mirroring `strip_po.py`. Does NOT match exit code 0 — that is the legitimate timer-done polling signal. Returns `(new_content, removed_chunks)` for `replaced_bg_completed_text` mod attribution.

**Wake-up architecture:** CC natively injects bg-task-completion signals when a background bash timer ends or is killed. Rather than discarding them (leaving Opus with an empty wake-up turn and no context), the proxy replaces the first signal with a plain-text hint that tells Opus workers may need attention. Plain text avoids `<system-reminder>` wrapping and the associated SR-strip-pass concerns.
**Reads:** Message content (string or list of blocks).
**Writes:** Nothing — returns `(modified_content, list[str])`.
**Called by:** `src/proxy/rules.py`
**Calls out:** stdlib only (`re`).
