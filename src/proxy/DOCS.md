...
**`tool_result.content` is strip-free by design.** System-Reminders are hook-injected into user-message text-blocks only. A `<system-reminder>` tag appearing in `tool_result.content` is always a code literal or user-visible text — never a real hook. Strip functions (`_strip_system_reminder`, `_strip_all_system_reminders`, `_strip_plan_mode_blocks`, `_strip_user_interrupt_sr`, `_strip_task_notification_tags`, `_content_contains`) operate exclusively on top-level string content and `{type: "text"}` blocks. Exception: `_strip_rejection_message` and `_message_has_rejection` intentionally access `tool_result.content` — rejection messages are actual tool_result responses, not injected SRs.
