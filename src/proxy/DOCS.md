...
**Purpose:** mitmproxy addon hook class (`ProxyAddon`) that receives HTTP flows and orchestrates the full request-modification and dual-log pipeline; `count_tokens` requests pass through unmodified; `request()` also forces `accept-encoding: identity` on the outbound Messages request so the response body arrives uncompressed for `response_model_probe`.
