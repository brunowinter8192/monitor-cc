...
ALWAYS-ON shape since the 2026-09-16 M6 redesign, having started 2026-09-10 as the opt-in `--turns`
...
timeline will not load. `reqs` (2026-09-04, redesigned 2026-09-16 as M6 — see process-docs/
...
turn-assignment rule a short-lived `turns` subcommand (2026-09-08/09, removed 2026-09-10) and then
the opt-in `reqs --turns` flag (2026-09-10, removed 2026-09-16) both used before it
...
**Calls out:** `discovery`, `render`, `search`, `timeline`, `overlay`, `usage`, `project_map` (all package-local; `overlay.build_overlay` from both `_run_expand` and `_run_msgs`; `overlay.build_sys_tool_overlay` only from `_run_msgs`; `usage.build_usage_by_flow` from `_run_msgs` always and from `_run_reqs` always (2026-09-16, M6 — was `--rebuild`/`--drop`-only before); `project_map.build_project_index` only from `_load_for`; `timeline.resolve_req_range` only from `_run_msgs`'s `--req` path; `discovery.filter_by_family` only from `_run_reqs`).
...
**Called by:** `__main__.py` (`_run_msgs` always and, since the 2026-09-16 M6 redesign, `_run_reqs` always too — CR/CC is now part of `reqs`' baseline line form, not a `--rebuild`/`--drop`-only extra), so `sessions`/`search`/`expand` are the only commands that never read `~/.claude/projects/`.
...
third element — a worker's name or a main session's label) is unchanged since 2026-09-10. And the
...
**Called by:** `__main__.py`.
**Calls out:** `timeline` (`request_markers`, for `msgs`' REQ separators; `_system_block_chars`/`_tool_chars`, since 2026-09-04, for a sys/tool line's original-chars lookup; `_BILLING_HEADER_SYS_INDEX`, same date, to exempt system index 0 from that lookup; `_group_markers_by_turn`/`_turn_preview`, since 2026-09-10, now for EVERY `reqs` call — see Purpose); `reader` (`local_datetime`, since 2026-09-04, for every timestamp this module renders); `discovery` (`stem_identity`, since 2026-09-10, for `_session_tag` only).
...
(2026-09-16, M6 — supersedes the ADD-output-tail design every entry below this one predates).**
...
more (2026-09-04, tail removed 2026-09-16).** Before M6, `REQ 206 18:10:11  +89m` spelled out the
...
(2026-09-16, M6 — this is a REGRESSION in cost relative to before, made deliberately).** Before M6,
...
2026-09-10, always-on since the 2026-09-16 M6 redesign — and a removed `turns` subcommand before
...
through an opt-in `--turns` flag from 2026-09-10 to 2026-09-16).** A `request_markers`
...
column — the send-time-only view `reqs --turns` (2026-09-10) then provided, and `reqs` itself
provides unconditionally since the 2026-09-16 M6 redesign (which also dropped the per-request
...
(`render.py`) all went with the 2026-09-10 pivot — see process-docs/dual_log_cli/ for the full
...
`render.py`) `_fmt_duration` — kept a second time by the 2026-09-16 M6 amendment specifically for
