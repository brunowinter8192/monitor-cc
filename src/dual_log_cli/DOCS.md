...
`turns`) accepts a substring of either the full stem or this displayed form, so a name copied out
of `sessions`' own SESSION column resolves.
...
**Called by:** `discovery.list_sessions`/`build_session`/`project_for_stem` (once per run, shared across all sessions), `__main__._load_for`, `usage.build_usage_by_flow`/`build_request_times_by_flow` (via `usage._resolve_session_transcript`) — `project_label` specifically by `discovery.project_for_stem` and `usage._candidate_dirs`.
