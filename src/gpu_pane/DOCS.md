...
**Reads:** `~/.rag-locks/server-port-*.json` JSON; `http://localhost:<port>/health`; `ps -o rss=`; log file mtime via `log_path` from state file; `rag-cli server presets --json` (once per process at import).
...
- click triggers `_fire_button` which sets `_toggle_state` to `starting`/`stopping` then runs `rag-cli` fire-and-forget; if the rag-cli execution stalls, label gets stuck until `TOGGLE_TIMEOUT=120s` natural expiry. (Bead `Monitor_CC-fg9d` closed 2026-05-10 after live-verify.)
