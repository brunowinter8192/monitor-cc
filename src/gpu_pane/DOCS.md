...
- **`status.py`'s `except PermissionError: pass` (PID alive, different owner) is written single-line.** The platform's write-time safety hook rejects a two-line `except ...:\n    pass` shape with no comment; the single-line form is the standing workaround (same one used in `ghostty.py` during the menubar milestone) and carries no comment.
