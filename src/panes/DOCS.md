...
- Zebra/hover/truncation render loop lives in `_build_tokens_output()` in `token_pane.py`, NOT in `token_format.py`. `format_cache_tracker` returns a 5-tuple `(visible_lines, visible_keys, sticky_header, viewport_start, initial_parent_count)` on the normal path; 4-tuple on empty turns (pre-existing edge-case: unpack will fail if called with `_cache_turns = []` before the first JSONL data arrives).
