...
def run_worker_metadata_loop() -> None:
    ...
    from .proxy_pane import find_worker_proxy_log, _parse_log_file
...
# Format metadata display for a worker proxy entry (uses separate state from main metadata pane)
def _format_worker_metadata(entry: dict) -> str:
    global _worker_prev_values
    prev = _worker_prev_values
    _worker_prev_values = {}
    result = _format_metadata_with_state(entry, prev, _worker_prev_values)
    return result

# Format metadata with explicit state dicts (allows both main and worker panes to use same logic)
