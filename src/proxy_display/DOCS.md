...
`render_system_blocks`: unchanged detection, per-block expand/collapse, change highlights. On expand (use_dual path): if `i_spans` is new-format (`isinstance(i_spans[0], (list, tuple))`), renders inline — iterates `[(tag, text), ...]`, equal=DIM gray, injected=DIM_GREEN_BG green, no separate gray preview; then `s_spans` (flat strings) as DIM_YELLOW_BG stacked below. Old-format or no i_spans: gray preview + stacked yellow s_spans + stacked green i_spans (legacy path, backward-compat). Header coloring (DIM_YELLOW_BG/DIM_GREEN_BG/both with `[STRIPPED]`/`[INJECTED]`/`[REPLACED]`) unchanged. Old path (no dual): hardcoded sys[2]/sys[3] detection via `mods`, yellow `original_text` from entry.

`render_tools`: new path resolves `s_tool`/`i_tool` per tool name. `i_tool.get('whole')` → DIM_GREEN_BG header `[INJECTED]` + green description bg (proxy-injected tool, whole-injected path unchanged). Desc-changes path: if `i_desc` is new-format, inline render (equal=DIM, injected=DIM_GREEN_BG), no forwarded-description block; `s_desc` (flat strings) stacked yellow below. Old-format i_desc: forwarded description + stacked yellow/green (legacy path). Whole-stripped extra rows: iterates `_stripped_spans['tools']` for `{'whole': true}` entries not in `tools_names` → DIM_YELLOW_BG `[STRIPPED]`. Old path: `stripped_original`, `stripped_unused_tools_names`.
