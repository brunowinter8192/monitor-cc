...
selected worker's proxy log (worker pane not yet migrated to dual-log overlay). Also exports
`parse_proxy_log` and path helpers used by `metadata` and `panes.warnings_pane`. Touch this
...
**Calls out:** `input` (click_handler), `workers` (worker_tmux, worker_pane.get_selection_file_path, write_selection), `panes` (token_pane.build_cache_turns), `utils` (visual_line_count)
...
**Called by:** `src/proxy_display/pane.py`, `src/proxy_display/worker_proxy_pane.py`, `src/panes/warnings_pane.py`, `src/metadata/metadata_pane.py`; `_find_dual_log_paths` + `accumulate_dual_log` + `_infer_model_family` called by `pane.py` only
**Calls out:** —
...
`render_system_blocks`: unchanged detection, per-block expand/collapse, change highlights. New path: `s_spans = entry['_stripped_spans']['system'].get(str(bidx))` / `i_spans = ...['injected']...` → header colored DIM_YELLOW_BG/DIM_GREEN_BG/both with `[STRIPPED]`/`[INJECTED]`/`[REPLACED]`; on expand shows description then yellow span block then green span block (stacked). Old path: hardcoded sys[2]/sys[3] detection via `'replaced_system_prompt'`/`'stripped_sys3'` in `mods`, yellow `original_text` from the entry.

`render_tools`: new path resolves `s_tool`/`i_tool` per tool name. `i_tool.get('whole')` → DIM_GREEN_BG header `[INJECTED]` + green description bg (proxy-injected tool, e.g. MCP). `s_tool.get('desc')`/`i_tool.get('desc')` → yellow/green span blocks after description. Whole-stripped extra rows: iterates `_stripped_spans['tools']` for `{'whole': true}` entries not in `tools_names` → DIM_YELLOW_BG `[STRIPPED]` rows. `deferred_tools_names` kept unconditionally (CC's own SR-block deferral, not from diff engine). Old path: `stripped_original` per tool-def, `stripped_unused_tools_names` list for extra rows.

`render_fields_delta` (NEW): collapsible `('fields', entry_idx)` header showing count of changed fields; when expanded, one line per key — yellow DIM_YELLOW_BG old value (stripped-only), green DIM_GREEN_BG new value (injected-only), yellow+green pair for replaced. No-ops when `_stripped_spans` absent or both fields dicts empty. Covers model-override, max_tokens, output_config, thinking.
...
### render_messages.py (263 LOC)

**Purpose:** Render new/modified/removed messages for an expanded request entry — handles added messages (full block content) and diffs (content_tail). Dual-color sentinel: `use_dual = '_stripped_spans' in entry`. New path: for each rendered `(msg_idx, bidx)` block, appends yellow span block from `entry['_stripped_spans']['messages'].get(str(msg_idx), {}).get(str(bidx), [])` (DIM_YELLOW_BG) then green span block from `entry['_injected_spans']['messages']...` (DIM_GREEN_BG) — stacked after block content. `_render_stripped_block` calls (old side-channel using `stripped_msg_removed`/`stripped_msg_originals`) and pre-loops for old stripped messages are guarded with `and not use_dual`; in new path all messages render with normal headers. **EFF:RULE attribution dropped in new path** — dual-logs store span texts only, no rule codes; out of scope per user decision. Old path unchanged: per-chunk `EFF:RULE` label (e.g. `EFF:NAG`, `EFF:CMD`) via `strip_vocab.attribute_chunk`; IDX case appends `IDX` inline on `[STRIPPED]` header. Also exports `_aggregate_entry_tags(entry)` (suspect-tag badge helper) — diff-aware: returns `[]` for byte-identical re-fires (`first_diff_index < 0`), treats `first_diff_index = None` as first REQ, otherwise filters `stripped_msg_removed` to indices ≥ `first_diff_index`. And `_aggregate_req_buckets(entry, prev_entry)` — thin delegate to `strip_vocab.classify_req` for per-REQ 5-bucket signals. Content rendering highlights 4 suspect tag literals with `LIGHT_RED_BG` via `_SUSPECT_TAG_RE`.
...
**Dual-log accumulator (pane.py only):**
- `_proxy_stripped_pos` / `_proxy_injected_pos` — byte-position cursors for incremental dual-log reads; reset to 0 on session change
- `_proxy_acc_stripped` / `_proxy_acc_injected` — `{family: {'system': {}, 'tools': {}, 'messages': {}, 'fields': {}}}` accumulator dicts; mutated IN-PLACE by `accumulate_dual_log` (`.clear()`+`.update()` on section dicts preserves Python references); cleared on session change via `.clear()`. Each newly-parsed entry holds a REFERENCE to its family's accumulator dict — NOT a copy. Render code reads span data from these references at render time, so late-arriving delta updates propagate automatically.
