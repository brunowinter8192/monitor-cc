...
def render_system_blocks(entry_idx: int, entry: dict, prev_entry_for_delta, expand_states: dict, pane_width: int) -> tuple:
    lines = []
    keys = []
    ...
    keys.append(sys_key)
    if is_sys_expanded:
        b_lines, b_keys = _render_sys_blocks_body(entry_idx, entry, sys_blocks, prev_entry_for_delta, expand_states)
        lines.extend(b_lines)
        keys.extend(b_keys)
    return lines, keys

def _render_sys_blocks_body(entry_idx: int, entry: dict, sys_blocks: list, prev_entry_for_delta, expand_states: dict) -> tuple:
    lines = []
    keys = []
    ...
    for sb in sys_blocks:
        if not is_first_sys and prev_block_by_idx.get(sb['idx'], {}).get('preview', '') == sb.get('preview', ''):
            continue
        blk_lines, blk_keys = _render_one_sys_block(entry_idx, entry, sb, expand_states)
...
def _render_one_sys_block(entry_idx: int, entry: dict, sb: dict, expand_states: dict) -> tuple:
    ...
    s_spans, i_spans = _sys_block_spans(entry, bidx)
    ...
    if is_block_expanded:
        c_lines, c_keys = _render_sys_block_content(sb, s_spans, i_spans)
...
def _render_sys_block_content(sb: dict, s_spans, i_spans) -> tuple:
    lines = []
    keys = []
    if i_spans and isinstance(i_spans[0], (list, tuple)):
