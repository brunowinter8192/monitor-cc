...
def launch_split_screen(project_filter: Optional[str] = None, ui: bool = False, script_path: str = '') -> None:
    ...
    # Window 3 "workers": Workers (left, 50%) + Subagents (right, 50%)
    ...
    subprocess.run(["tmux", "split-window", "-h", "-t", f"{session_name}:3.0", "-l", "50%", subagents_cmd])
...
def configure_tmux_session(session_name: str) -> None:
    ...
    subprocess.run(["tmux", "bind-key", "-T", "root", "M-s", "run-shell", f"tmux capture-pane -t {session_name}:3.1 -pS - | pbcopy && tmux display 'Subagents pane copied'"])
