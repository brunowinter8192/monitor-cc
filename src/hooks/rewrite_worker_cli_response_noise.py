...
# Anchor: only `worker-cli response` — `capture`, `status`, `list`, `send`, `merge`,
# `spawn`, `kill`, `revive` do not contain the word `response` and cannot match.
_WCLI_RE = re.compile(r'\bworker-cli\s+response\b')

# Segment-end operators: terminate the worker-cli response logical command.
...
# Read Bash tool_input from stdin; for each `worker-cli response` invocation,
# strip any downstream pipes/redirects/2>&1 inside its logical segment.
# Chains around the segment (cd && ..., ... ; bd list) are preserved.
def rewrite_worker_cli_response_noise_workflow() -> None:
    ...
    log_fire("rewrite_worker_cli_response_noise", "rewrite", "Bash", command,
...
if __name__ == "__main__":
    rewrite_worker_cli_response_noise_workflow()
