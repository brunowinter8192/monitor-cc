...
def _parse_command():
    try:
        ...
        log_fire("block_git_destructive", "trace", "Bash", "", reason=f"parse error: {type(e).__name__}: {e}")
        return None, None
        ...
        if c in ("'", '"'):
            ...
            while i < n and s[i] != quote:
                if s[i] == "\\" and i + 1 < n:
                    ...
                    i += 1
            if i >= n:
                log_fire("block_git_destructive", "trace", "Bash", s, reason="unterminated quote: remainder dropped")
            i += 1
        else:
            out.append(c)
            i += 1
