...
**Purpose:** Position-preserving shell-region stripper — blanks heredoc bodies, single/double-quoted strings, and ANSI-C `$'...'` quotes to same-length spaces before pattern matching, keeping `$(...)` and backtick command substitutions active; on a deliberate `_StripError` (unclosed heredoc or quote) it returns the input unchanged and logs a `trace` line; any other exception propagates.
...
**Purpose:** `log_fire(hook_name, decision, tool_name, command, ...)` appends one JSON line per hook decision; creates the log directory, and on write failure prints one stderr line and returns so logging never breaks a hook.
...
- **`log_fire` decision values:** `"block"` (exit 2 + stderr, `reason` field), `"rewrite"` (exit 0 + updatedInput, `rewritten` field) and `"trace"` (`reason` field, no effect on the hook outcome: parse errors, raw-text strip fallback, exempted shlex segments, unknown-size po block, degraded worker-status check, rag repeat-state failures, getcwd failure) are used by current modules; filter traces with `jq 'select(.decision != "trace")'`; a third value `"ui-notice"` is reserved and unused. Filter it out of any fire-log analysis with `jq 'select(.decision != "ui-notice")'`.
