# Strand conversion of the proxy_176 tests — 2026-09-24

Converted: `proxy_176_agent_types_tests.py` (6 strands), `proxy_176_strip_tests.py` (9 strands), `proxy_176_bg_launch_ack_tests/proxy_176_bg_launch_ack_tests.py` (24 strands; the test functions live in the sibling `*_cases.py` and `*_cases_w3.py` modules and are found through the runner's `globals()`; `proxy_176_bg_launch_ack_report.py` now holds the raising `check`). `test_composition_invariant.py` was left as it is: it is one loop over one frozen corpus and already tripwires on a missing fixture, so there are no independent cases to split.
