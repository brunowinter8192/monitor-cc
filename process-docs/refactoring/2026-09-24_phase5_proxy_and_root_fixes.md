...
## Menubar import cycle (defect from the resolver commit, fixed later)

`menubar_log` imports `paths`; `paths` resolves `MONITOR_CC_ROOT` at import and the reporter I handed it imported `menubar_log` lazily. Importing `src.menubar.menubar_log` first (which `import src.menubar` does through `monitor_sweep_scheduler`/`system`) therefore re-entered a partially initialized `menubar_log` and raised `ImportError: cannot import name 'log_menubar'`. My earlier test loaded `root_report` through a fake package and never imported the real chain, and the import order I reasoned about (paths first) was the one that works. Fix: `root_report.report_root(log_path, root, source)` writes the line itself (same format as `log_menubar`, OSError swallowed like `log_menubar` does) and `paths.py` binds the log path with `functools.partial`. `dev/monitor_root/test_monitor_root.py` now imports `src.menubar`, `src.menubar.menubar_log` and `src.menubar.paths` each in a fresh interpreter. Lesson: an import-time callback into a module that imports you is a cycle whichever side starts; test by importing every entry point in a fresh interpreter.
