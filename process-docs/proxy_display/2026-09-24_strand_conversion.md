# Strand conversion and verification declarations in dev/proxy_display — 2026-09-24

Converted: `test_req_prefix_turn_headers.py` (10 strands, 55 checks), `test_standalone_sidecar.py` (2 strands, 8 checks).

Declared as verification only (DOCS.md Kind line, code unchanged): `render_byte_identity.py` (live forwarded log, pin with `RENDER_BYTE_IDENTITY_LOG_DIR`) and `verify_req_numbering.py` (B16: live dual-log directory and `~/.claude/projects` at hardcoded paths, no env seam, arguments stem and turn).
