...
1. `apply_modification_rules()` — Content-Modifications:
   ...
   - `trimmed_task_notification` (completed) / `replaced_task_notification` (failed): **identische Output-Logik** — beide ersetzen den gesamten `<task-notification>`-Block durch einen **einzigen** Wake-up-Block: `_WAKEUP_TEXT` + optionale `Output: <path>`-Zeile wenn die TN ein `<output-file>`-Tag enthält. `<summary>`-Text und `<status>`-Zeile werden verworfen (nicht forwarded). Implementierung: `_replace_task_notification_tags(old_content, injected_text)` ersetzt den TN-Block inline im bestehenden text-Block; `_extract_task_notification_output_file(old_content)` extrahiert den Pfad. Detection via `_top_level_content_contains` (payload_helpers.py) — prüft NUR top-level str/text-blocks, steigt NICHT in `tool_result`-Content ab. Mod-Name-Unterschied ist rein für Logging/Attribution: `trimmed_task_notification` = completed-Status, `replaced_task_notification` = failed-Status.
   ...
   - **CC 2.1.176 Strips (2026-06):**
     - `stripped_role_system_msg` (`_apply_role_system_strip`, pass 1 / erster in `_passes`): Ersetzt Content jeder `role=='system'`-Message mit `"."`. CC 2.1.176 liefert deferred-tools-Liste, agent-types-Liste und skills-Liste als standalone `role='system'`-Message (~9,559 chars, kein `<system-reminder>`-Wrapper) statt als `text`-Block in einer `role='user'`-Message. Idempotenz-Guard: überspringt leere und bereits `"."`-Messages. Attribution role-basiert (nicht content-marker-basiert): `_process_messages_section` in `strip_inject_delta.py` weist `code='RS'` zu wenn `om_norm.role=='system'`. strip_vocab.py Rule `'RS'` mit leerem Marker-Array.
