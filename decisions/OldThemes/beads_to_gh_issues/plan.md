...
Bead 4e1b. Decision 2026-06-02: replace beads/dolt entirely with GitHub Issues across all projects, then decommission beads. Sequenced AFTER the naming-unification structure migration (Bead uoyx) — runs in a FRESH session.
...
4. **Remove everything bead-related from the machine**, EXCEPT the OldThemes about beads (`decisions/OldThemes/bd_mutation_revert/`, `dolt_server_lifecycle.md`, this folder): dolt sql-servers, per-project `.beads/`, the `bd` CLI, bd hooks, bd references in workflow/rules.

## Per-project export volume (from 4e1b, migration sizing)
