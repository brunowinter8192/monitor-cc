...
**Note on cache-creation across cascade REQs (inferred, not observed):** We have no direct measurement of `cache_creation_input_tokens` for aborted REQs — their usage data is never written to session JSONL. Inferred from the streaming-API structure (`usage` delivered in `message_start`, see sources above): cache_creation is likely billed at stream-open even when the stream is aborted mid-content, but this is not confirmed by observation. Worst case: `N × cache_creation` tokens are billed and each aborted REQ writes a cache entry that the next REQ cannot fully reuse (because appending TN blocks shifts the breakpoint position); best case: only the completed REQ's cache_creation is billed and aborted streams incur only cache_read costs. Real billing behavior would need verification via the Anthropic usage API or billing dashboard.
