...
- ⏳ **2E remaining:** (b) SR/TN header badges (#4) — `modifications=[]` always, badges gone; design options: re-source from `_stripped_spans`, synthesize from fn_maps, or drop; (c) full live-verify on new-format logs needs proxy restart
