...
- ✅ **Badge false-positive fix** — three phantom `fn_map` sources eliminated (`logging.py` + `strip_sr.py` + `parser.py`); verified on real logs (4 field + 2 dot + 2 nl-phantom removed; 48 bg-exit + 3 real strips preserved). **`fn_map` attribution rules:** (1) field overrides (`field.*` loc_keys — model, max_tokens, thinking, output_config, context_management) NOT written to `fn_map` at write-side in `_build_stripped_injected_deltas`; they appear only in `fields_delta`; (2) message blocks whose only injected text is `"."` (empty-block placeholder, `strip_sr.py` `or '.'`) NOT written to `fn_map`; (3) `strip_sr.py` partial mode now preserves original trailing-`\n` — no longer forwards a net-new `\n` when the original SR had none. Read-side `accumulate_dual_log` reverted to `set(fn_map.values())` (no `field.*` filter needed). Detail: `decisions/OldThemes/proxy_tool_stripping/17_badge_false_positives.md`
