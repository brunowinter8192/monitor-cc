...
1. `apply_modification_rules()` — Content-Modifications:
   ...
   - `trimmed_task_notification`: Strippt output-file/tool-use-id Tags aus task-notifications (non-failed TN only)
   - `replaced_task_notification`: Ersetzt `<task-notification>` Blöcke mit `<status>failed</status>` durch `'worker idle\n'` (`_WAKEUP_TEXT` in `strip_bg_completed.py`). Wake-up-Hint für Opus wenn CC einen Background-Bash-Timer signalisiert.
   - `replaced_bg_completed_text` (`strip_bg_completed.py`): Ersetzt die erste CC-native BG-Exit-Notification (plain-text kill-signal format) durch `'worker idle\n'`. Weitere BG-Notifications werden gestrippt. Getrennter Pfad von `<task-notification>` XML.
