...
1. `apply_modification_rules()` — Content-Modifications:
   ...
   - `trimmed_task_notification`: Non-failed (completed) TN — ersetzt den gesamten `<task-notification>`-Block durch einen **einzigen** Wake-up-Block: `_WAKEUP_TEXT` + optionale `Output: <path>`-Zeile wenn die TN ein `<output-file>`-Tag enthält. `<summary>`-Text wird verworfen (nicht forwarded). Implementierung: `_replace_task_notification_tags(old_content, injected_text)` ersetzt den TN-Block inline im bestehenden text-Block (kein separater Append); `_extract_task_notification_output_file(old_content)` extrahiert den Pfad. Kein Aufruf von `_append_wakeup_text_to_content` — verhindert den Double-Block-Bug (summary-Block + wakeup-Block). Detection via `_top_level_content_contains` (payload_helpers.py) — prüft NUR top-level str/text-blocks, steigt NICHT in `tool_result`-Content ab. Verhindert false-positive wenn `<task-notification>` als DATA in einem tool_result auftaucht.
