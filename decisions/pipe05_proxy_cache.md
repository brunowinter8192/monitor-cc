...
1. `apply_modification_rules()` — Content-Modifications:
   ...
   - **CC 2.1.176 Strips (2026-06):**
     ...
     - `stripped_bg_launch_ack` (`_apply_bg_launch_ack_strip` via `strip_bg_launch_ack.py`): Ersetzt Content aller Blöcke die die Background-Command-Launch-Ack enthalten mit `"."`. Marker: `'running in background with ID'`. Deckt alle 4 Content-Shapes ab (str-Message, text-Block, tool_result-str, tool_result-list). strip_vocab.py Rule `'BL'`. Pass nach `_apply_bg_exit_strip` in `_passes`.
