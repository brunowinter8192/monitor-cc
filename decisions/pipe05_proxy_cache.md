...
Zusätzlich schreibt `addon.py` fünf additive Logs in `src/logs/dual_log/` (Subfolder, auto-created):
...
- `api_requests_<log_id>_response.jsonl` — Response-HTTP-Header-Log. Geschrieben im `responseheaders()`-Hook für ALLE Status-Codes (kein 2xx-Gate — 429 `retry-after` muss erfasst werden). Gefiltert via `_filter_response_headers` (exact: `request-id`, `retry-after`, `anthropic-organization-id`; prefix-match: `anthropic-ratelimit-*`, `anthropic-priority-*`, `anthropic-fast-*`; Keys normalisiert zu lowercase). Entry-Felder: `flow_id`, `timestamp`, `request_id` (aus Response-Header `request-id`), `status_code`, `headers` (gefiltertes Dict). Korrelation zu `_forwarded`/`_stripped`/`_injected` via `flow_id`. Writer-Handle: `self.response_log_file = _resolve_dual_log_file("response")`.

Alle fünf Writes je in eigenem `try/except` — Fehler beeinflussen nie Forwarding oder Main-Log. `_stripped`/`_injected` werden vom `response()`-Hook geschrieben (Metadata-Bridge: `mc_original_payload`, `mc_modified_payload`, `mc_model_family` auf `flow.metadata`). Janitor-Rotation: `dual_log/`-Files werden quintet-aligned rotiert — nach der Haupt-Log-Rotation (count-30 opus/worker) werden alle `dual_log/`-Files gelöscht, deren `log_id` nicht mehr unter den überlebenden Haupt-Logs ist. Implementiert in `_janitor_cleanup_jsonl_logs()` (`claude_proxy_start.sh`); alle fünf Suffixe in `_LOG_REGISTRY` (`log_janitor.py`) mit `retention="count-30"`, `janitor_trigger="proxy-start-bash"`.

**Korrelations-Key:** Alle fünf Dual-Log-Entries tragen `"flow_id": flow.id` — mitmproxys stabiles Per-Flow-UUID, das über alle Hooks identisch ist. `request_id` in `_response` kommt aus dem Anthropic-Response-Header `request-id` (direkt verfügbar in `responseheaders()`). `flow_id` ist der Read-Side-Join-Key für alle fünf Logs.
