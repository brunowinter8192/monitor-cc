...
1. `apply_modification_rules()` — Content-Modifications:
   ...
   - `trimmed_task_notification`: Strippt output-file/tool-use-id Tags aus task-notifications (non-failed TN) UND appended `_WAKEUP_TEXT` ans Content (Verhalten ab 2026-05-22 commit `fcfe6c1` — vorher nur strip ohne wake-up, was zu "." Platzhalter-Dots im outgoing Payload führte ohne Hinweis was war).
   - `replaced_task_notification`: Ersetzt `<task-notification>` Blöcke mit `<status>failed</status>` durch `_WAKEUP_TEXT` (`'background done — check worker or other process\n'`). Generischer Wake-up-Hint für jede Art von Background-Task-Completion oder Kill.
   - `replaced_bg_completed_text` (`strip_bg_completed.py`): Ersetzt die erste CC-native BG-Exit-Notification (plain-text kill-signal format) durch `_WAKEUP_TEXT` (`'background done — check worker or other process\n'`). Weitere BG-Notifications werden gestrippt. Getrennter Pfad von `<task-notification>` XML.
