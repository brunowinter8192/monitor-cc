...
4. **Restart button — fixed** (2026-05-28): `restartApp_` gates on `sys.frozen`; py2app branch calls `write_plist_py2app()` (writes `ProgramArguments = [.../Monitor_CC_Menubar]`) then pure launchctl bootout+bootstrap — no bundle rebuild, no Python invocation in helper. See `decisions/OldThemes/menubar_restart_broken/A2_fix.md`. Dev-mode restart footgun also fixed (2026-06-12): `setup_menubar_workflow` removed from `setup_menubar.py`; dev branch now uses identical pure launchctl cycle — no bundle rebuild possible.
