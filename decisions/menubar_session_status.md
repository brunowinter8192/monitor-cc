...
- Session name reconstructed from worker JSONL cwd: split on `/.claude/worktrees/`, `basename(left)` = project basename. **Worker name** (`worker_name` part) comes from `os.path.basename(cwd)` — NOT from the `~/.claude/projects/` encoded-dir name. `encode_project_path` replaces `_` with `-` (lossy); encoded-dir-derived names corrupt underscores → tmux name mismatch → worker dropped. `basename(cwd)` preserves underscores exactly. `SessionInfo.name` (display) uses the same `basename(cwd)` value.
- **`project_name` (group-header) from live cwd basename** (`discover.py:_process_project_dir`): `project_name = os.path.basename(cwd.partition('/.claude/worktrees/')[0]) or project_name` — set inside the `if cwd and '/.claude/worktrees/' in cwd:` block. Overrides the `_decode_dir_name` value from `_classify_encoded_dir`. Ensures the header matches the live project dir name.
