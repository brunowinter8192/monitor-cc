...
- Alive fallback (cwd unreadable from JSONL): `ALIVE_WINDOW_SECS=3600s` JSONL age guard; display name falls back to lossy encoded-dir name (no tmux check in this branch, so mismatch is moot).
