# dev/dual_log_cli/tests/

## Role
Synthetic-fixture regression suite for `src/dual_log_cli/`: msgs, reqs, search, sessions, skip reporting and pane-numbering rules, proven by calling the real functions. Touch when changing `src/dual_log_cli/` behavior; do not add fixtures requiring a live monitor root or a real Claude projects tree.

## Public Interface
No `__init__.py`. Each script is run directly, e.g. `./venv/bin/python dev/dual_log_cli/tests/test_reqs.py`.

## Flow
A test builds synthetic dicts or writes a temp forwarded-shaped file, runs the real function under test and compares against an expected value; a failing check raises so its strand aborts.
Converted suites run as parallel fail-fast strands through the strand runner in `dev/refactoring/`.

## Modules

### test_local_time.py (109 LOC)
...
### test_msgs_req_range.py (118 LOC)
...
### test_msgs_sys_delta.py (171 LOC)
...
### test_msgs_sys_tool_overlay.py (170 LOC)
...
### test_project_display.py (192 LOC)
...
### test_reqs_rebuild_drop.py (172 LOC)
...
### test_reqs_turn_and_family.py (121 LOC)
...
### test_sidecar_exclusion.py (138 LOC)
...
### test_skip_reporting.py (190 LOC)
...
### test_tool_name_comparison.py (159 LOC)
...
### test_reqs_pane_numbering.py (55 LOC)
...
### reqs_pane_numbering_fixtures.py (268 LOC)
...
### reqs_pane_numbering_basic_checks.py (150 LOC)
...
### reqs_pane_numbering_ownership_checks.py (117 LOC)
...
### reqs_pane_numbering_unmapped_checks.py (40 LOC)
