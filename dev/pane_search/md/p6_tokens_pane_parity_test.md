# P6 — tokens pane search bar parity regression suite
...
[shape] Tokens pane search state is one search_bar.SearchState, lowercase label
  PASS  _tokens_search is a search_bar.SearchState instance
  PASS  label is 'search: ' (lowercase, visual consistency with the majority of panes)
  PASS  search bar is fixed 1-line

## PASS test_search_bar_row1_renders_no_arrows
...
[matches] Editing (plain backspace, kill-line) never clears _tokens_search.matches — Enter remains the sole recompute trigger
...
[match: call, collapsed] Whole call-header line container-marked even though the match text lives in unrendered (collapsed) detail content
  PASS  call is NOT expanded
  PASS  Enter reports a change
  PASS  real search found the call (force-expand matcher sees collapsed content)
...
[match: call, expanded] Header stays container-marked AND the specific matching detail line gets browser-find substring-highlighted
  ...
  PASS  header line STILL container-marked when expanded (uniform, orientation-preserving)
...
[match: turn] A match in the turn's own prompt line gets the turn header container-marked
  ...
  PASS  no ('turn', 0) key leaked into cache_line_map (turn headers stay non-interactive)
...
[sentinel fix] Same bug class as the proxy pane: ZEBRA_BG_A=='' — the sentinel must resolve to an explicit \033[49m on a detail line, not be deleted outright (which would flood the search highlight color to the rest of the row)
...
[regression] LIGHT_RED_BG (cc_broken row) detection uses 'in line', not '.startswith()' — a search-match wrap now precedes it in the string
  PASS  real search found the (also cc_broken) call
...
[jump] Enter jumps cache_scroll_offset to bring an off-screen early match into view
  ...
  PASS  jump pushed cache_scroll_offset above 0 (turn 0 is far from the default bottom view)

## PASS test_session_change_resets_search_state


[worker switch parity] Session change resets _tokens_search and _tokens_nav — mirrors pane.py's session-change reset and the fix applied to the main pane and the worker-proxy pane
  PASS  search state populated before the session change
  PASS  session actually changed
  PASS  query cleared by the session change
  PASS  matches cleared by the session change
  PASS  focused cleared by the session change
  PASS  _tokens_nav cleared by the session change
