# Ground-Truth Message Spans Probe — 2026-06-05 22:27:05
...
| TEXT_REPLACE (msg[0] blk[0] DEF-SR → '.') | 504 | 1 | 1 | True | ❌(o=False,f=True) | ✅ | 1 | 1 | RECORDING_GAP: blk[2] ENV-SR (len=373) not in stripped_msg_removed[0] |
| BG_REPLACE (msg[78] blk[0] TN→wakeup, 2 chunks) | 406 | 47 | 2 | True | ❌(o=False,f=True) | ⚠️(o=False,f=True) | 1 | 1 | NESTED_CHUNK(len=77) 'Background command "sleep 600 &amp;&amp;...' |
...
#### Bug-case phantom analysis
- GT has phantom on `'", "is_error": false}'`: False ✅ absent
- diff has phantom on `'", "is_error": false}'`: False ✅ absent
...
```
  ...
  ('injected', '.')
```
Fidelity: orig_ok=False fwd_ok=True ❌ LOSS
Injected spans: 1 — '.'
Zero phantom-like injected in GT ✅
...
**GT spans lossless on all 4 cases:** ❌ NO — see per-case details
