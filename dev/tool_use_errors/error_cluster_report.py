...
def format_report(entries: list, buckets: dict, cc: dict, report_date: str) -> str:
    ...
    lines += _build_cluster_table_section(buckets, total)
    lines += _build_exit_code_distribution_section(buckets)
    lines += _build_cross_check_section(cc)
    lines += _build_bare_guidance_section(buckets)
    lines += _build_sample_entries_section(buckets)
    lines += _build_conclusion_section(cc)
    return "\n".join(lines) + "\n"


def _build_cluster_table_section(buckets: dict, total: int) -> list:
    lines = [
...
def _build_exit_code_distribution_section(buckets: dict) -> list:
...
def _build_cross_check_section(cc: dict) -> list:
    lines = [
...
def _build_bare_guidance_section(buckets: dict) -> list:
    lines = [
...
def _build_sample_entries_section(buckets: dict) -> list:
    lines = [
...
def _build_conclusion_section(cc: dict) -> list:
    return [
        "## Conclusion",
        "",
