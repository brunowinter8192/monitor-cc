...
**Reads:** real `src/menubar` controllers built with a fake app and fake status items under an isolated HOME; `json/p2_panel_snapshot_before.json` and `..._after.json` in compare mode.
**Writes:** `json/p2_panel_snapshot_<label>.json` (with `--label`); `md/p2_panel_snapshot.md` (compare mode); nothing is shown on screen.
...
## State
None. Each script is stateless; reports in `md/` are overwritten on every run. `json/` holds the two committed panel snapshots (`before` taken on the code before the shared side-panel refactor, `after` on the refactored code).
